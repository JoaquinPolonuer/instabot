...
accounts = input(
    "ingresar cuentas similares separadas por un espacio:").split()
comentarios = input(
    "ingresar comentarios separados por guiones bajos: ").split("_")
...
if platform == "linux" or platform == "linux2":
    ...
    session = InstaPy(username=user, password=passwd,
                      headless_browser=True, geckodriver_path='./geckodriver')
else:
    direc = user+"-"+str(time.time())
    os.mkdir(direc)
    set_workspace(os.getcwd()+"/"+direc)
